...
public class MyFoodListAdapter extends BaseAdapter {
    ...
    public MyFoodListAdapter(Activity baseActivity) {
    ...
    @Override
    public int getCount() {
        return FoodContent.foods.size();
    }

    @Override
    public Object getItem(int position) {
        return FoodContent.foods.get(position);
    }
...
//        return RecipeContent.recipes.get(position).id.hashCode();
    ...
    public View getView(int position, View convertView, ViewGroup container) {
        if (convertView == null) {
            convertView = LayoutInflater.from(activity).inflate(R.layout.list_item_food, container, false);
        }
//        final RecipeContent.RecipeItem item = (RecipeContent.RecipeItem) getItem(position);
        ((TextView) convertView.findViewById(R.id.recipe_title)).setText(FoodContent.foods.get(position).getName());
        ((TextView) convertView.findViewById(R.id.recipe_calories)).setText(FoodContent.foods.get(position).getDescription());
        ((TextView) convertView.findViewById(R.id.label)).setText(FoodContent.foods.get(position).getType());
//        final ImageView img = (ImageView) convertView.findViewById(R.id.thumbnail);
//        try {
//            Glide.with(activity)
//                    .load(FoodContent.foods.get(position).getImages().get(0))
//                    .asBitmap()
